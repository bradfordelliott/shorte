...
class list_item_t:
    ...
    def set_text(self, text):
        ...
        self.textblock = textblock_t(text)

    def get_text(self):

        return self.textblock.source.strip()

    def __str__(self, indent=0):

        output = ""
        lines = self.textblock.source.split("\n")
        index = 0
        for line in lines:
            if(index == 0):
                output += "%s- [%s] (indent: %d)\n" % (" "*indent, line, indent)
...
class textblock_t(object):
    ...
    TYPE_TABLE        = 5
    TYPE_INLINE_TAG   = 6
    TYPE_TEXTBLOCK    = 7 # A nested textblock
    ...
    def translate_type(self, inp):
        if(inp == textblock_t.TYPE_TEXT):
            ...
            return "table"
        elif(inp == textblock_t.TYPE_INLINE_TAG):
            return "tag"
        elif(inp == textblock_t.TYPE_TEXTBLOCK):
            return "textblock"
        ...
        while(i < end):
            ...
            if(state == STATE_NORMAL):
                ...
                if(source[i] == '@' and source[i+1] == '{'):
                    items.append(item)
                    item = ['@', '{']
                    states.append(STATE_INLINE_FORMATTING)
                    i += 2
                    continue
                    ...
                    if(len(item) != 0):
                        litem = list_item_t()
                        item = "".join(item)
                        item = item.replace(" "*(item_indent+1), "")
                        litem.set_text(item)
            ...
            elif(state == STATE_INLINE_FORMATTING):
                if(source[i] == '}'):
                    state = STATE_NORMAL
    ...
    def parse(self, data):
        ...
        while(i < len(data)):
            ...
            if(state == STATE_NORMAL):
                ...
                elif((i == 0 or data[i-1] == "\n") and (i < len(data)-1) and ((data[i].isdigit()) and data[i+1:i+3] == ". ")):
                    segments.append(segment)
                    segment = {}
                ...
                # Start of a new segment if we hit a blank line
                ...
                # Blocks in ```...``` are considered code segments
                elif(data[i:i+3] == "```"):
                    segments.append(segment)
                    segment = {}
                    segment["type"] = "code"
                ...
                # Anything that starts with @{ is considered a inline tag. This is mostly
                # used for inline styling like inserting tables or images within a textblock.
                elif(data[i] == '@' and data[i+1] == "{"):
                    #segments.append(segment)
                    #segment = {}
                    #segment["type"] = "text"
                ...
                # If the line starts with > then it is a quote block
                ...
                # Anything else should be added to the current segment.
                else:
                    #print "In Else block"
                    segment["text"] += data[i]
                    i += 1
            
            # If we're in an inline tag then continue until we hit the closing
            # } marker
        ...
        for segment in segments:
            ...
            if(type == "code"):
                ...
                output = []
                for line in lines:
                    matches = re.search(r"^(\s*)[0-9]+\.", line)
                    if(matches != None):
    ...
    def __str__(self):
        ...
        for paragraph in self.paragraphs:
            ...
            if(ptype in (textblock_t.TYPE_TEXT, textblock_t.TYPE_CODE, textblock_t.TYPE_QUOTE)):
                ...
                for line in lines:
                    output += "    [%s]\n" % line
            elif(ptype in (textblock_t.TYPE_ORDERED_LIST, textblock_t.TYPE_LIST)):
                output += "    data:\n"
                items = paragraph["text"]
                for item in items:
                    output += item.__str__(6)
            elif(ptype == textblock_t.TYPE_TEXTBLOCK):
...
#}
#
#This is a list with some nested code blocks
#
#- One
#  
#      A nested code block
...
#    - One A
#        
#        A nested code block inside a list item
#- Two
#"""
    tb = textblock_t(text)
    print tb
